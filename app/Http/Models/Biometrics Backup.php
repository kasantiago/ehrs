		 ...
		 if($status == 'PERMANENT'){
		 	switch (strtolower($day)) {
		 		case 'mon':
						...
						$ppm_time_in = new DateTime('13:00:00');
...
// ers INNER JOIN biologs ON users.biometric_id = biologs.IndRegID) as biometric_data WHERE biologs_user_id = 15 AND date = '2019-01-25'
     ...
     public static function morning_in($date,$user_id,$biometric){
     	   ...
     	   $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND  TIME_FORMAT(biologs.TimeOnlyRecord,'%p')  = 'AM' AND biologs.DwInOutMode = 0 AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord ASC  LIMIT 1";
     		...
     		if($query){
     			return date('h:i', strtotime($query[0]->time ));
			}else{
      ...
      public static function morning_out($date,$user_id,$ain,$pin,$biometric){
      	   ...
      	   if($ain){
      	   	$ain = " AND biologs.TimeOnlyRecord > '".$ain."' AND TIME_FORMAT(biologs.TimeOnlyRecord,'%H:%i') != '".date('h:i', strtotime($ain))."'";
      	   }

      	   if($pin){
      	   	$pin = " AND biologs.TimeOnlyRecord < '".$pin."' AND TIME_FORMAT(biologs.TimeOnlyRecord,'%H:%i') != '".date('h:i', strtotime($pin))."'";
      	   }

     	   $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."' AND  biologs.DwInOutMode = 1  AND biologs.TimeOnlyRecord < '13:00' ".$pin."".$ain."  AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord ASC LIMIT 1"; //ASC
     		...
     		if($query){
     			...
     			$query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND biologs.TimeOnlyRecord < '13:00' ".$pin."".$ain."  AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord ASC LIMIT 1"; //ASC
     	   ...
     	   $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND  TIME_FORMAT(biologs.TimeOnlyRecord,'%p')  = 'PM' AND biologs.DwInOutMode = 0 AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord ASC  LIMIT 1";
     		...
     		if($query){
     			return  date('h:i', strtotime($query[0]->time));
     			  ...
     			  $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND  TIME_FORMAT(biologs.TimeOnlyRecord,'%p')  = 'PM' AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord ASC  LIMIT 1,1";
     ...
     public static function afternoon_out($date,$user_id,$pm_in,$biometric){
      	   ...
      	   if($pm_in){
      	   	$pm_in = " AND biologs.TimeOnlyRecord > '".$pm_in."' AND TIME_FORMAT(biologs.TimeOnlyRecord,'%H:%i') != '".date('h:i', strtotime($pm_in))."'";
      	   }



     	   $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND  TIME_FORMAT(biologs.TimeOnlyRecord,'%p')  = 'PM' AND biologs.DwInOutMode = 1 ".$pm_in." AND biologs.TimeOnlyRecord > '13:00'  AND biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord DESC  LIMIT 1";
     		...
     		if($query){
     		    ...
     		    $query = "SELECT biologs.TimeOnlyRecord as time FROM users INNER JOIN biologs ON users.biometric_id = biologs.IndRegID WHERE users.id = ".$user_id." AND biologs.DateOnlyRecord = '".$date."'  AND  TIME_FORMAT(biologs.TimeOnlyRecord,'%p')  = 'PM'  ".$pm_in." AND biologs.TimeOnlyRecord > '13:00'  AND  biologs.MachineNumber = '".$biometric."' ORDER BY biologs.TimeOnlyRecord DESC  LIMIT 1";
     		    $query = DB::select($query);
