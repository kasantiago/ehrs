@extends('layouts.app')
@section('title','PERSONAL INFORMATION DATABASE')
                          ...
                          @if(App\Http\Models\AdminRequest::system_settings() == 1)
                            ...
                            <div class="row clearfix">
                                <div class="col-md-3" style="border-right: 2px solid #f0f0f0;">
                                    <form method="POST" id="send_request_to_all" action="{{ url('personal-data-sheet-table/send-request-to-all') }}">
                                        <button type="submit" class="btn {{ $color[2] }} waves-effect col-md-12">SEND REQUEST TO ALL</button>
                                    </form>
                                </div>

                                <form method="POST" id="send_selected" action="{{ url('personal-data-sheet-table/send-request-selected-unit') }}">
                                <div class="col-md-6">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.SendRequestForm').on('click',function(){
          ...
          $('#user_id').val(user_id);
          $('#formSubmit').attr('action', 'personal-data-sheet-table/send-request');
          $('#SendRequestFormLabel').html("Do you want to send request (PDS) to "+name+"");
        });
